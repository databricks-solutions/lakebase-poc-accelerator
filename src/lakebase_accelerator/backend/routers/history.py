...
class HistoryConnIn(BaseModel):
    ...
    # target schema for the history table — a dedicated, SP-owned schema by default
    schema_name: str = history.DEFAULT_SCHEMA


class HistorySaveIn(HistoryConnIn):
    label: str | None = None
...
def enable_lakebase_history(req: HistoryConnIn, request: Request) -> HistoryEnableOut:
    """Preflight the SP's privileges and create the history table (consent step)."""
    try:
        creds = _sp_creds(req, request)
    except Exception as e:  # noqa: BLE001
        return HistoryEnableOut(ok=False, message=f"Could not resolve connection: {e}", error=str(e), ddl=_safe_ddl(req.schema_name))

    try:
        result = history.ensure_table(creds, req.schema_name)
        return HistoryEnableOut(**result, ddl=_safe_ddl(req.schema_name))
        ...
        return HistoryEnableOut(
            ...
            message=(
                ...
                f"A project owner must provision a least-privilege role + owned schema (below), or use "
                f"the 'This browser' destination which needs no setup."
            ),
            grant_sql=history.provisioning_sql(sp, req.schema_name, creds.database, include_role=True),
            ddl=_safe_ddl(req.schema_name),
            error=str(e),
        )
    except Exception as e:  # noqa: BLE001
        logger.info(f"history enable failed: {e}")
        return HistoryEnableOut(ok=False, message="Could not enable Lakebase history", error=str(e), ddl=_safe_ddl(req.schema_name))


@router.post("/history/lakebase/save", response_model=HistorySaveOut, operation_id="saveLakebaseHistory")
def save_lakebase_history(req: HistorySaveIn, request: Request, ws: EffectiveClient) -> HistorySaveOut:
    """Insert one run (baseline + optimized) under the SP, attributed to the OBO user."""
    try:
        creds = _sp_creds(req, request)
        run_id = history.save_run(
            ...
            optimized_report=req.optimized_report,
            index_ddls=req.index_ddls,
        )
        return HistorySaveOut(ok=True, id=run_id)
    except Exception as e:  # noqa: BLE001
        logger.info(f"history save failed: {e}")
        return HistorySaveOut(ok=False, error=str(e))
...
def list_lakebase_history(req: HistoryConnIn, request: Request) -> HistoryListOut:
    """List recent runs from the Lakebase history table (shared, newest first)."""
    try:
        creds = _sp_creds(req, request)
        runs = history.list_runs(creds, req.schema_name)
...
def _safe_ddl(schema_name: str) -> str | None:
    try:
        return history.ddl(schema_name)
