...
const ConcurrencyTestingPsycopgAutoscaling: React.FC = () => {
...
SELECT c_preferred_cust_flag, count(*) FROM customer group by c_preferred_cust_flag;`
  ...
  const handleFileUpload = useCallback(async (file: File) => {
    ...
    try {
      console.log('Sending request to /api/concurrency-test/autoscaling/upload-query');
      const response = await fetch('/api/concurrency-test/autoscaling/upload-query', {
        method: 'POST',
        body: formData,
      });
  ...
  const handleRunTest = async () => {
    try {
      console.log('Button clicked! Starting autoscaling test execution...');
      ...
      console.log('Sending autoscaling test config:', testConfig);

      const endpoint = querySource === 'predefined'
        ? '/api/concurrency-test/autoscaling/run-predefined-tests'
        : '/api/concurrency-test/autoscaling/run-uploaded-tests';
      ...
      setTestError(null);
      message.success(`Autoscaling concurrency test completed successfully!`);
    ...
    <div style={{ padding: '24px', maxWidth: '1200px', margin: '0 auto' }}>
      <div style={{ marginBottom: '24px' }}>
        <Title level={2} style={{ marginBottom: '8px' }}>
          ...
          Autoscaling Concurrency Testing (psycopg)
        </Title>
            ...
            <Radio.Group
              ...
              onChange={async (e) => {
                ...
                if (newSource === 'predefined') {
                  setUploadedFiles([]);
                  try {
                    await fetch('/api/concurrency-test/autoscaling/clear-temp-files', { method: 'POST' });
                  } catch (error) {
                ...
                <div style={{ marginTop: '24px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        try {
                          const response = await fetch('/api/concurrency-test/autoscaling/clear-temp-files', {
                            method: 'POST'
                          });
                      ...
                      >
                        ...
                        <div style={{ marginBottom: '8px' }}>
                          <Button
                            ...
                            onClick={async () => {
                              try {
                                const response = await fetch('/api/concurrency-test/autoscaling/delete-query', {
                                  method: 'DELETE',
...
export default ConcurrencyTestingPsycopgAutoscaling;
